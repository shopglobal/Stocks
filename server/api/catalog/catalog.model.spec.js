...
describe('Stock+Catalog', function() {
  ...
  it('should create a stock with book catetory', function(done) {
    Stock.create({title: 'Broccoli', price: 123, categories: [books._id]})
      .then(function () {
        return Stock.findOne({}).populate(['categories', 'categories.parent']);
      })
...
describe('Finding stocks by category', function () {
  var mainCatalog, electronics, books, clothing, vehicle, pantry, appliances;

  var createStocksCatalog = function (callback) {
    Catalog
      .find({})
      .remove()
      .then(function () {
  ...
  beforeEach(function (done) {
    return createStocksCatalog(done);
  });

  it('should find 3 electronic stocks', function(done) {
    Stock
      .find({'categories': electronics })
      .populate('categories')
      .exec()
      .then(function (stocks) {
        stocks.length.should.be.equal(3);
        done()
      })
      .then(null, done);
  });

  it('should find one vehicle and one clothing stocks', function (done) {
    Stock
      .find({'categories': { $in: [vehicle, clothing]} })
      .populate('categories')
      .exec()
      .then(function (stocks) {
        stocks.length.should.be.equal(2);
        done()
      })
      .then(null, done);
  });
  ...
  it('should find _all_ stocks', function (done) {
    Stock
      ...
      .then(function (stocks) {
        ...
        stocks.length.should.be.equal(6);
        done()
      })
      .then(null, done);
  });
